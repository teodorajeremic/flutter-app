...
class EnergyBurnedPage extends StatefulWidget {
  const EnergyBurnedPage({super.key});

  @override
  State<EnergyBurnedPage> createState() => _EnergyBurnedPageState();
}

class EnergyBurnedEntry {
...
class _EnergyBurnedPageState extends State<EnergyBurnedPage>
    with SingleTickerProviderStateMixin {
  List<EnergyBurnedEntry> entries = [];
  List<EnergyBurnedEntry> filteredEntries = [];
  ...
  Future<void> _loadFromBackend() async {
    try {
      ...
      item.containsKey("cycle_start_time") &&
          item.containsKey("energy_burned_cal") &&
          item["energy_burned_cal"] != null &&
          item["cycle_start_time"] != null)
          .map((item) {
        String raw = item["cycle_start_time"].toString().replaceAll(" ", "T");
        DateTime parsedDate = DateTime.parse(raw);
        double temp =
            double.tryParse(item["energy_burned_cal"].toString()) ?? 0;
        return EnergyBurnedEntry(
            startDate: parsedDate, EnergyBurned: temp);
      }).toList();
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                entry.EnergyBurned == 0
                    ? '--'
                    : entry.EnergyBurned.toStringAsFixed(0),
                style: TextStyle(
                  fontSize: 15,
                  fontWeight: FontWeight.w600,
                  color: entry.EnergyBurned > 0
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: SfCartesianChart(
                    ...
                    series: <SplineAreaSeries<EnergyBurnedEntry, DateTime>>[
                      SplineAreaSeries<EnergyBurnedEntry, DateTime>(
                        dataSource: filteredEntries,
                        xValueMapper: (e, _) => e.startDate,
                        yValueMapper: (e, _) =>
                        e.EnergyBurned == 0 ? null : e.EnergyBurned,
